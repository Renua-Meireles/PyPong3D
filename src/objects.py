...
class Table(Rectangle):
    def __init__(self, color, rotation, position, dimension):
        ...
        self.border_top =    Rectangle(colors.TABLE_BORDER, rotation, (x, y+dy_border, z+dz_border), (w, border_h, border_d))
        self.border_left =   Rectangle(colors.TABLE_BORDER, rotation, (x-dx_border, y, z+dz_border), (border_w, h, border_d))
        self.border_right =  Rectangle(colors.TABLE_BORDER, rotation, (x+dx_border, y, z+dz_border), (border_w, h, border_d))
        self.border_bottom = Rectangle(colors.TABLE_BORDER, rotation, (x, y-dy_border, z+dz_border), (w, border_h, border_d))

        leg_w, leg_h, leg_d = 0.3, 0.3, 2
        dx_leg, dy_leg, dz_leg = w+leg_w/2, h+leg_h/2, d+leg_d/2
        self.leg_top_left =     Rectangle(colors.TABLE_LEG, rotation, (x-dx_leg, y+dy_leg, z-dz_leg), (leg_w, leg_h, leg_d))
        self.leg_top_right =    Rectangle(colors.TABLE_LEG, rotation, (x+dx_leg, y+dy_leg, z-dz_leg), (leg_w, leg_h, leg_d))
        self.leg_bottom_left =  Rectangle(colors.TABLE_LEG, rotation, (x-dx_leg, y-dy_leg, z-dz_leg), (leg_w, leg_h, leg_d))
        self.leg_bottom_right = Rectangle(colors.TABLE_LEG, rotation, (x+dx_leg, y-dy_leg, z-dz_leg), (leg_w, leg_h, leg_d))
...
class Scene(Obj3d):
    def __init__(self, rotation, position):
        ...
        w_p, d_p = 0.1, 0.1
        self.player1 = Rectangle(colors.PLAYER1, (0, 0, 0), (self.table.limit_left+w_p,  y, z+self.table.d+d_p), (w_p, 0.4, d_p))
        self.player2 = Rectangle(colors.PLAYER2, (0, 0, 0), (self.table.limit_right-w_p, y, z+self.table.d+d_p), (w_p, 0.4, d_p))

        w_g, d_g = 15, 0.1
        self.ground =     Rectangle(colors.GROUND, (0, 0, 0), (x, y, z-abs(self.table.limit_depth)-d_g), (w_g, w_g, d_g))

        d_w = 10
        self.wall_back =  Rectangle(colors.WALL,   (0, 0, 0), (    x, y-w_g, z-abs(self.table.limit_depth)+d_w), (w_g, d_g, d_w))
        self.wall_front = Rectangle(colors.WALL,   (0, 0, 0), (    x, y+w_g, z-abs(self.table.limit_depth)+d_w), (w_g, d_g, d_w))
        self.wall_left =  Rectangle(colors.WALL,   (0, 0, 0), (x-w_g,     y, z-abs(self.table.limit_depth)+d_w), (d_g, w_g, d_w))
        self.wall_right = Rectangle(colors.WALL,   (0, 0, 0), (x+w_g,     y, z-abs(self.table.limit_depth)+d_w), (d_g, w_g, d_w))

        self.roof = Rectangle(colors.WALL, (0, 0, 0), (x, y, z-abs(self.table.limit_depth)+d_g+self.wall_back.d), (w_g, w_g, d_g))


        self.objects = [
            self.table, self.ball, self.player1, self.player2,
            self.ground, self.wall_back, self.wall_front, self.wall_left, self.wall_right, self.roof
        ]
